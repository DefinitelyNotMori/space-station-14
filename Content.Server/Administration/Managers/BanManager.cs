...
public sealed partial class BanManager : IBanManager, IPostInjectInit
{
    ...
    public bool IsRoleBanned(ICommonSession player, IReadOnlyList<ProtoId<JobPrototype>> jobs)
    {
        return IsRoleBanned<JobPrototype>(player, jobs);
    }

    public bool IsRoleBanned(ICommonSession player, IReadOnlyList<ProtoId<AntagPrototype>> antags)
    {
        return IsRoleBanned<AntagPrototype>(player, antags);
    }

    private bool IsRoleBanned<T>(ICommonSession player, IReadOnlyList<ProtoId<T>> roles) where T : class, IPrototype
    {
